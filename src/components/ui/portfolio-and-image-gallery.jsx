  ...
  (
    ...
    useEffect(() => {
      ...
      const observer = new ResizeObserver((entries) => {
        ...
        if (hasChanged && typeof ScrollTrigger !== 'undefined') {
    ...
    const calculatedBuffer = extraBuffer !== undefined
      ? extraBuffer
      : (childSize ? childSize.h * scaleFactor - childSize.h + 120 : 180);

    const visibleAreaHeight = childSize
      ? circleDiameter * visibleDecimal + childSize.h + calculatedBuffer
      : circleDiameter * visibleDecimal + 300;

    const computedRotation = rotationProgress !== undefined ? rotationProgress * 360 : undefined;

    return (
      <div
        ref={mergedRef}
        className={`w-full relative flex items-center justify-center overflow-visible ${className}`}
        {...rest}
      >
        <div
        ...
        >
          <ul
            ...
            style={{
              ...
              bottom: -(circleDiameter * hiddenDecimal * 0.2),
              transformOrigin: 'center center',
              transform: computedRotation !== undefined ? `rotate(${computedRotation}deg)` : undefined,
            }}
          >
            {childrenNodes.map((child, index) => {
              // Offset by -Math.PI / 2 so the first items sit at top-center of the visible arc
              const angle = -Math.PI / 2 + (index / childrenCount) * 2 * Math.PI;
